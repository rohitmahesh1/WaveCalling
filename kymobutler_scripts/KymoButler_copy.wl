...
BiKymoButlerTrack[pred_, kym_, kympreproc_, bool_, binthresh_, vthr_, vismod_, minSz_, minFr_, debug_: False, indicesInput_: Null] := Module[
    {dim = ImageDimensions@kym, coloredlines, overlay, overlaylabeled, labels, c, ptmp, sel, ovlpIDs, out, paths, pathstmp, inflp, t, seedbin, seeds, allyx, ptrk, trks, checkifAnyTrkisSubset, ovlpSegID, filteredTrks, indicesList},
    ...
    Sow[First@AbsoluteTiming[
        ...
        seeds = SortBy[Map[Abs[{dim[[2]] + 1, 0} - #] &, Reverse /@ PixelValuePositions[seedbin, 1]], First];
        allyx = SortBy[Map[Abs[{dim[[2]] + 1, 0} - #] &, Reverse /@ PixelValuePositions[paths, 1]], First];
        If[debug, Sow@HighlightImage[paths, seedbin]];
    ], "skeletonization"];
    ...
    If[Length@trks > 0,
        ...
        (* Apply minimum frame requirement *)
        trks = Select[trks, First@Last@# - First@First@# >= minFr &];

        (* Handle indices *)
        indicesList = If[indicesInput === Null, Range[Length@trks], indicesInput];
        indicesList = Select[indicesList, 1 <= # <= Length@trks &];
        Print[indicesList];

        (* Filter the tracks based on the passed indices *)
        filteredTrks = trks[[indicesList]];

        Sow[First@AbsoluteTiming[
            (* Colored lines and overlays *)
            coloredlines = ImageRotate[
                Image[
                    Show[
                        Image@Table[0, dim[[1]], dim[[2]]],
                        Graphics@Map[
                            {
                                RandomColor[], 
                                Style[Line@#, Antialiasing -> False]
                            } &,
                            Map[# - {1, 0} &, filteredTrks, {2}]
                        ]
                    ],
                    ImageSize -> Reverse@dim
                ],
                -Pi/2
            ];
        ...
        overlay = ImageCompose[ImageAdjust@kym, RemoveBackground[coloredlines, {Black, .01}]];
            ...
            (* Get labels and label overlay *)
            c = SparseArray[
                Flatten[MapIndexed[Thread[#1 -> First@#2] &, filteredTrks], 1],
                {dim[[2]], dim[[1]]}
            ];
            labels = ComponentMeasurements[c, "Centroid"];

            overlaylabeled = HighlightImage[
                overlay,
                MapIndexed[
                    Function[{lbl, idx},
                        ImageMarker[
                            lbl[[2]] + {0, 5},
                            Graphics[{If[bool, White, Red], Text[Style[ToString[indicesList[[First@idx]]], FontSize -> Scaled@.03]]}]
                        ]
                    ],
                    labels
                ]
        ...
        {kym, coloredlines, overlay, overlaylabeled, filteredTrks},
...
BiKymoButler[kym_, binthresh_, vthr_, tD_, cnet_, vismod_, minSz_, minFr_, debug_ : False, indices_ : Null] := 
 
 Module[{rawkym, kympreproc, pred, bool, t, out},
  {t, {bool, rawkym, kympreproc, pred}} = AbsoluteTiming @ BiKymoButlerSegment[kym, cnet, tD];
  If[debug, Sow[pred, "pmap"]];
  Sow[t, "seg"];
  {t, out} = AbsoluteTiming @ BiKymoButlerTrack[pred, rawkym, kympreproc, bool, binthresh, vthr, vismod, minSz, minFr, debug, indices];

  Sow[t, "tracking"];
  If[indices =!= {},
    AppendTo[out, BiKymoGraph[out, indices]],
    AppendTo[out, BiKymoGraph[out]]
  ]; 
  out
