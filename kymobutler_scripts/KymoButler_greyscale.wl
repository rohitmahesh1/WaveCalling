...
BiKymoButlerTrack[pred_, kym_, kympreproc_, bool_, binthresh_, vthr_, vismod_, minSz_, minFr_, debug_: False] := Module[
    ...
    coloredlines = ImageRotate[
        Image[
            Show[
                ...
                Graphics@Map[{RandomColor[], Style[Line@#, Antialiasing -> False]} &, Map[# - {1, 0} &, trks, {2}]]
            ],
    ...
    (* Modify the overlay creation to use the original colored image *)
    overlay = ImageCompose[kym, RemoveBackground[coloredlines, {Black, .01}]];
    
    (* Ensure the labels are overlaid on the same image *)
    dim = ImageDimensions[overlay];
    imageWidth = dim[[1]];
    imageHeight = dim[[2]];
    ...
    {
        labels[[#, 2]][[1]],
        imageHeight - labels[[#, 2]][[2]] + 5
    } &,
    Range[Length@labels]
    ];

    labelsGraphics = Graphics[
    Map[
        Text[
            Style[
                ToString@#,
    ...
    PlotRange -> {{0, imageWidth}, {0, imageHeight}},
    Background -> None,
    ImageSize -> {imageWidth, imageHeight},
    AspectRatio -> imageHeight / imageWidth
    ];

    overlaylabeled = ImageCompose[overlay, labelsGraphics];
    ],"colorize"];
    (* Return the results *)
